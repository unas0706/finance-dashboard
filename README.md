...
├── tests/
│   ├── helpers/
│   │   └── db.js               # In-memory MongoDB for tests
│   ├── auth.test.js
│   ├── records.test.js
│   └── dashboard.test.js
├── .env.example
...
# 4. (Optional) Seed the database with test users and records
npm run seed
...
Test files:
- `tests/auth.test.js` — Registration, login, /me endpoint
- `tests/records.test.js` — CRUD, RBAC enforcement, filtering, pagination
- `tests/dashboard.test.js` — Aggregation correctness, role access
